...
RO16_MATCHUPS = [
    # Vuitens de final - Mundial 2026 (4-8 juliol 2026)
    ("France", "Paraguay"),
    ("Canada", "Morocco"),
    ("Spain", "Portugal"),
    ("Belgium", "USA"),
...
def main():
    print("Generant Premium PDF en català...")
    ...
    note = ("Nota Tècnica: Aquest model avalua el rendiment col·lectiu i la força de bloc. "
            "No és possible predir golejadors individuals, ja que el sistema no rastreja "
            "l'estadística individual dels jugadors, sinó l'eficiència agregada de l'equip.")
    # Fix for fpdf2 standard fonts avoiding some utf-8 chars error if not using a unicode font: 
    # fpdf2 helvetica natively handles latin-1 including à, è, í, ó, ú, l·l, ç
    pdf.multi_cell(0, 5, note, fill=True, border=1)
    pdf.ln(8)
    
    for match_idx, (t1_name, t2_name) in enumerate(RO16_MATCHUPS, 1):
        ...
        pdf.cell(0, 8, f"PARTIT {match_idx}: {t1_name} vs {t2_name}", fill=True, new_x=XPos.LMARGIN, new_y=YPos.NEXT, align="C")
    ...
    print("Calculant probabilitats de guanyar el mundial des de Vuitens...")
    try:
        ...
        winners = []
        # Utilitzem RO16_MATCHUPS definit a l'inici del fitxer com a punt de partida
        for i in range(100000): # 100.000 iteracions d'alta precisió per la nit
            champion = simulate_bracket(RO16_MATCHUPS)
            if champion:
    ...
    output_path = PROJECT_ROOT / "Prediccions_Vuitens_CAT_WC2026.pdf"
