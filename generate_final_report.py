...
FINAL_MATCHUPS = [
    # Tercer i Quart lloc - Mundial 2026 (18 juliol 2026)
    ("England", "France"),
    # Final - Mundial 2026 (19 juliol 2026)
    ("Argentina", "Spain"),
]
...
def main():
    print("Generant Premium PDF de la Final en català...")
    ...
    for match_idx, (t1_name, t2_name) in enumerate(FINAL_MATCHUPS, 1):
        ...
        if match_idx == 1:
            match_title = f"TERCER I QUART LLOC: {t1_name} vs {t2_name}"
        else:
            match_title = f"GRAN FINAL: {t1_name} vs {t2_name}"
            pdf.set_fill_color(255, 215, 0) # Gold per a la final
        pdf.cell(0, 8, match_title, fill=True, new_x=XPos.LMARGIN, new_y=YPos.NEXT, align="C")
    ...
    # Add Final Win Probabilities
    pdf.add_page()
    pdf.set_font("helvetica", "B", 14)
    pdf.set_text_color(15, 23, 42)
    pdf.cell(0, 10, "PROBABILITAT DE GUANYAR EL MUNDIAL", new_x=XPos.LMARGIN, new_y=YPos.NEXT, align="C")
    
    t1_final, t2_final = FINAL_MATCHUPS[1]
    if t1_final != "Team_Winner_SF1" and t2_final != "Team_Winner_SF2":
        team1 = get_live_team(live_ranks, t1_final)
        team2 = get_live_team(live_ranks, t2_final)
        _, _, p_h, p_d, _, _, _ = predict_match(team1, team2, pipeline, stage_weight=1)
        prob_win = p_h + p_d * 0.5
        
        pdf.set_font("helvetica", "", 11)
        pdf.set_text_color(51, 65, 85)
        
        pdf.cell(100, 6, t1_final, border=1)
        pdf.cell(50, 6, f"{prob_win*100:.1f}%", border=1, align="R", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        
        pdf.cell(100, 6, t2_final, border=1)
        pdf.cell(50, 6, f"{(1 - prob_win)*100:.1f}%", border=1, align="R", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        
    output_path = PROJECT_ROOT / "Prediccions_Final_CAT_WC2026.pdf"
